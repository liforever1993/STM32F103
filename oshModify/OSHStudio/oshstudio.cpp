    ...
    ui(new Ui::OSHStudio)
{
    ...
    connect (ui->comboBox_BoardType, SIGNAL(currentIndexChanged(int)), SLOT(showBoardType(int)));
      ...
      QStringList list=(QStringList()<<"未使用"<<"Analog No Smoothing"<<"Analog Low Smoothing"
      ...
      ui->comboBox_BoardType->addItem("蓝色板");
      ui->comboBox_BoardType->addItem("黑色板");
        ...
        for(uint8_t i =0; i < 16; i++) {
            ...
            if (comboBox_ptr) {
                ...
                switch (curr_pin_type) {
                    ...
                    AxisComb1+=QChar::fromLatin1(port)+QString::number(i);
                    NumberAnalogInputs ++;
                    break;
